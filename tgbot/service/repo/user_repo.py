...
class UserRepo(BaseSQLAlchemyRepo):
    ...
    async def add_user(self, user_id: int, name: str, username: str, language: str):
        sql = insert(self.model).values(user_id=user_id, name=name, username=username, language=language).returning('*')
